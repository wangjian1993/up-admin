<template>
	<div class="card">
		<a-tabs default-active-key="1" @change="callback" size="small">
			<a-tab-pane key="1" tab="BOM费用-查询发布"><bom-issue @onSearch="searchClick" :searchDisabled="searchDisabled"></bom-issue></a-tab-pane>
			<a-tab-pane key="2" tab="BOM费用-发布记录"><bom-record @onSearch="searchClick"></bom-record></a-tab-pane>
		</a-tabs>
		<div class="loading" v-if="serachLod"><a-spin tip="加载中..." /></div>
		<details-list
			v-if="isSearch"
		...
		<footer-tool-bar>
			...
			<a-button type="primary" @click="bomForm">{{ tabIndex == 1 ? '提交' : '更新' }}</a-button>
		</footer-tool-bar>
	</div>
</template>
<script>
