 ...
 * @LastEditTime: 2022-10-20 10:03:36
 * @LastEditors: max
 * @Description: 
 * @FilePath: /up-admin/src/pages/srm/market/client/component/inviteDetail.vue
-->
<template>
  <div>
    <a-drawer :visible="visible" title="供应商信息" placement="right" @close="close" :get-container="false" :wrap-style="{ position: 'absolute' }" width="100%" :footer="null" centered :bodyStyle="{ padding: '5px 10px' }">
      <a-tabs type="card">
        ...
        <a-tab-pane key="2" tab="我的绩效"> </a-tab-pane>
      </a-tabs>
    </a-drawer>
  </div>
</template>
...
<script>
import { getClientList } from "@/services/srm.js";
...
export default {
  ...
  methods: {
    ...
    getDetailList() {
      ...
      getClientList(params, "single").then((res) => {
        if (res.data.success) {
          this.supplierInfo = res.data.data.client;
