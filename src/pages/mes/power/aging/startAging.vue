 ...
 * @LastEditTime: 2022-10-28 11:48:58
 * @LastEditors: max
 * @Description: 
 * @FilePath: /up-admin/src/pages/mes/power/aging/startAging.vue
-->
<template>
  <!-- 搜索 -->
...
<script>
import { getProcessAging } from "@/services/mes.js";
...
export default {
  ...
  methods: {
    ...
    getWorkInfo() {
      getProcessAging("", "loaduserline").then((res) => {
        if (res.data.success) {
    ...
    scanCode(e) {
      ...
      getProcessAging(params, "scan").then((res) => {
        res.data.message.time = getTimeData();
        this.orderList = [];
        if (res.data.success) {
          this.AgeingQty = 0;
    ...
    startWork() {
      ...
      getProcessAging(params, "submit").then((res) => {
        res.data.message.time = getTimeData();
        if (res.data.success) {
          res.data.message.IsSuccess = res.data.data.IsSuccess;
          if (res.data.data.IsSuccess) {
