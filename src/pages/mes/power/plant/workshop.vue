 ...
 * @LastEditTime: 2022-10-12 10:34:04
 * @LastEditors: max
 * @Description: 
 * @FilePath: /up-admin/src/pages/mes/power/plant/workshop.vue
-->
<template>
    <div>
      <a-spin tip="导出中..." :spinning="isExportLod">
        ...
        <workshopForm v-if="isForm" :isEdit="isEdit" :editData="editData" :plantList="plantList" @closeModal="closeModal" @success="searchBtn" />
      </a-spin>
    </div>
  </template>
  
  <script>
  import { getPowerPlant, setPowerPlant } from "@/services/mes.js";
    ...
    methods: {
      ...
      search() {
        this.searchForm.validateFields((err, values) => {
          if (!err) {
            ...
            getPowerPlant(params, "workshop/getall").then((res) => {
              if (res.data.success) {
      ...
      allDel() {
        ...
        self.$confirm({
          title: "确定要删除选中内容",
          onOk() {
            setPowerPlant(self.selectedRowKeys, "workshop/delete").then((res) => {
              if (res.data.success) {
      ...
      onDelete(item) {
        ...
        setPowerPlant(params, "workshop/delete").then((res) => {
          if (res.data.success) {
