<template>
  <div>
    <a-modal :title="isEdit ? '编辑车间配置' : '添加车间配置'" v-if="visible" :visible="visible" @ok="handleOk" @cancel="handleCancel">
...
<script>
import { setPowerPlant } from "@/services/mes.js";
export default {
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          if (this.isEdit) {
            ...
            setPowerPlant(editForm, "workshop/update").then((res) => {
              if (res.data.success) {
            ...
            //添加
            setPowerPlant(this.form, "workshop/add").then((res) => {
              if (res.data.success) {
