...
<template>
  ...
  <a-card class="card" :bordered="false" :bodyStyle="{ padding: '5px' }">
    ...
    <div class="tab">
      ...
      <ProcessForm v-if="isForm" :isEdit="isEdit" :editData="editData" :plantList="plantList" @closeModal="closeModal" @success="searchBtn" />
    </div>
  </a-card>
</template>
<script>
import { getProcess, setProcess } from "@/services/mes.js";
import { columns } from "./data/process";
  ...
  methods: {
    ...
    allDel() {
      ...
      self.$confirm({
        title: "确定要删除选中内容",
        onOk() {
          setProcess(self.selectedRowKeys, "delete").then((res) => {
            if (res.data.success) {
    ...
    onDelete(item) {
      let params = [];
      params.push(item.Id);
      setProcess(params, "delete").then((res) => {
        if (res.data.success) {
