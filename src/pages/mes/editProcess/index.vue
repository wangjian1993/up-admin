 ...
 * @LastEditTime: 2023-06-20 14:40:47
 * @LastEditors: max
 * @Description: 
 * @FilePath: /up-admin/src/pages/mes/editProcess/index.vue
-->
<template>
  <div>
      ...
      <a-form layout="horizontal" :form="searchForm">
        <div :class="advanced ? null : 'fold'">
          <a-row>
            ...
            <a-col :md="6" :sm="24">
              <a-form-item label="进站报工日期" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
...
<script>
import { getProcess, editProduction ,getEditProduction } from "@/services/mes.js";
  ...
  methods: {
    ...
    search() {
      this.searchForm.validateFields((err, values) => {
        if (!err) {
          ...
          getEditProduction(params, "getall").then((res) => {
            if (res.data.success) {
    ...
    allDel() {
      ...
      self.$confirm({
        title: "确定要删除选中内容",
        onOk() {
          editProduction(self.selectedRowKeys, "delete").then((res) => {
            if (res.data.success) {
    ...
    onDelete(item) {
      ...
      editProduction(params, "delete").then((res) => {
        if (res.data.success) {
