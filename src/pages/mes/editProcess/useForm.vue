 ...
 * @LastEditTime: 2023-06-20 14:47:36
 * @LastEditors: max
 * @Description: 
 * @FilePath: /up-admin/src/pages/mes/editProcess/useForm.vue
-->
<template>
  <div>
...
<script>
import { editProduction } from "@/services/mes.js";
export default {
  props: ["editData"],
  data() {
    return {
    ...
    handleOk() {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          editProduction(params, "update").then((res) => {
            if (res.data.success) {
              this.$emit("closeModal");
              this.$emit("succeed");
