 ...
 * @LastEditTime: 2023-05-04 11:31:26
 * @LastEditors: max
 * @Description: 
 * @FilePath: /up-admin/src/pages/mes/push/device/form.vue
-->
<template>
  <div>
    <a-modal v-if="visible" :title="isEdit ? '编辑设备' : '添加设备'" :visible="visible" destoryOnClose @ok="handleOk" @cancel="handleCancel" :width="840">
      <a-form-model ref="ruleForm" :model="form" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
        ...
        <a-row>
          <a-col :span="12">
            <a-form-model-item ref="lineid" has-feedback label="生产产线" :labelCol="{ span: 6 }">
...
import { setKanBanDevice } from "@/services/mes.js";
export default {
  ...
  created() {
    ...
    if (this.editData) {
      this.form = {
        ...
        equipmenttype: this.editData.EquipmentType,
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          if (this.isEdit) {
            ...
            setKanBanDevice(this.form, "update").then((res) => {
              if (res.data.success) {
            ...
            this.form.EquipmentType = "看板显示屏";
            setKanBanDevice(this.form, "add").then((res) => {
              if (res.data.success) {
