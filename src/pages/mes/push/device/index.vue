<template>
  <div>
    <a-card class="card" :bordered="false" :bodyStyle="{ padding: '5px' }">
      ...
      <Form v-if="isForm" :editData="editData" :isEdit="isEdit" :paramsItem="paramsItem" @close="close" @success="success" />
    </a-card>
  </div>
</template>
  ...
  methods: {
    ...
    allDel() {
      ...
      self.$confirm({
        title: "确定要删除选中内容",
        onOk() {
          setKanBanDevice(self.selectedRowKeys, "delete").then((res) => {
            if (res.data.success) {
    ...
    useDelete(item) {
      ...
      setKanBanDevice(params, "delete").then((res) => {
        if (res.data.success) {
