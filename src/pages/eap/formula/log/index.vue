...
<script>
import { getFormulaAction, getDeviceTypeAction, setDeviceAction } from "@/services/eap.js";
  ...
  methods: {
    ...
    allDel() {
      ...
      self.$confirm({
        title: "确定要删除选中内容",
        onOk() {
          setDeviceAction(self.selectedRowKeys, "log/delete").then((res) => {
            if (res.data.success) {
    ...
    onDelete(item) {
      ...
      setDeviceAction(parmas, "log/delete").then((res) => {
        if (res.data.success) {
