    ...
    <x-card v-if="hasPerm('sysApp:page')">
      <div slot="content" class="table-page-search-wrapper">
        <a-form layout="inline">
          <a-row :gutter="48">
            <a-col :md="8" :sm="24">
              <a-form-item label="应用名称"><a-input v-model="queryParam.name" allow-clear placeholder="请输入应用名称" /></a-form-item>
            </a-col>
            <a-col :md="8" :sm="24">
              <a-form-item label="唯一编码"><a-input v-model="queryParam.code" allow-clear placeholder="请输入唯一编码" /></a-form-item>
            </a-col>
            <a-col :md="8" :sm="24">
    ...
    <a-card :bordered="false">
      <a-spin :spinning="loading">
        <s-table ref="table" :columns="columns" :data="loadData" :alert="false" :rowKey="record => record.id">
          ...
          <span slot="active" slot-scope="text">{{ activeFilter(text) }}</span>
          ...
          <span slot="action" slot-scope="text, record">
            ...
            <a-popconfirm v-if="hasPerm('sysApp:delete')" placement="topRight" title="确认删除？" @confirm="() => sysAppDelete(record)"><a>删除</a></a-popconfirm>
            <a-divider
              type="vertical"
              v-if="
                hasPerm('sysApp:setAsDefault') & hasPerm('sysApp:delete') & (record.active == 'N') ||
                  hasPerm('sysApp:edit') & hasPerm('sysApp:setAsDefault') & (record.active == 'N')
              "
            />
