 ...
 * @LastEditTime: 2022-02-10 17:33:50
 * @LastEditors: max
 * @Description: 
    ...
    <a-card class="card" :bordered="false" :bodyStyle="{ padding: '5px' }">
      <a-spin tip="导出中..." :spinning="isExportLod">
        ...
        <div class="operator">
          ...
          <a-button v-if="hasPerm('shipment')" icon="edit" type="primary" :disabled="!hasSelected" :loading="loading" @click="onShipment()" style="margin-left: 8px">确认出货</a-button>
          <a-button v-else icon="edit" type="primary" disabled :loading="loading" @click="allDel" style="margin-left: 8px">确认出货</a-button>
        ...
        >
          ...
          <template slot="action" slot-scope="text, record">
            <div>
              <a-popconfirm title="确认出货?" @confirm="() => onShipment(record, 'Radio')" v-if="record.IsShipment == 'N'">
                <a style="margin-right: 8px" :disabled="!hasPerm('shipment')">
                  <a-icon type="edit" />
                  确认出货
                </a>
              </a-popconfirm>
...
export default {
  ...
  methods: {
    ...
    getListAll() {
      ...
      getOrderInfoAgc(parmas, "getplantshipping").then((res) => {
        if (res.data.success) {
    ...
    onShipment(item, type) {
      ...
      if (type == "Radio") {
        ...
        confirmShipmentAgc(parmas).then((res) => {
          if (res.data.success) {
