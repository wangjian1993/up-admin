    ...
    <a-card class="card" :bordered="false" :bodyStyle="{ padding: '5px' }">
      ...
      <a-descriptions layout="vertical" bordered size="small" :column="9">
        ...
        <a-descriptions-item label="约定验货时间">
          <a-date-picker style="width: 130px" :disabled-date="disabledDate" v-model="saveParmas.GoodsExamineTime" />
        </a-descriptions-item>
        <a-descriptions-item label="约定提货时间">
          <a-date-picker style="width: 130px" :disabled-date="disabledDate1" v-model="saveParmas.GoodsPickTime" />
        </a-descriptions-item>
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      this.saveParmas.PiNumber = this.orderInfo.DOC_NO;
      this.saveParmas.CustomerCode = this.orderInfo.CUSTOMER_CODE;
      this.saveParmas.ToCountry = this.orderInfo.TO_COUNTRY;
      //验货时间
      if (this.saveParmas.GoodsExamineTime != "") {
        this.saveParmas.GoodsExamineTime = this.saveParmas.GoodsExamineTime.format("YYYY-MM-DD");
      }
      //提货时间
      this.saveParmas.GoodsPickTime = this.saveParmas.GoodsPickTime.format("YYYY-MM-DD");
      this.data.map((item) => {
      ...
      setShipment(this.saveParmas).then((res) => {
        if (res.data.success) {
          this.reset();
