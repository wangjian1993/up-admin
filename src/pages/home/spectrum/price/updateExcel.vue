<template>
  <div>
    <a-modal v-model="visible" title="更新数据" @cancel="close" @ok="handleOk" :maskClosable="false" centered :width="400">
...
export default {
  ...
  methods: {
    ...
    submitExcel(params) {
      this.isUpload = true;
      setSpectrumPrice(params, "updateall").then((res) => {
        if (res.data.success) {
    ...
    readFile(file) {
      ...
      const userRelations = {
        品牌: "Brand", //行号
        品号: "ItemCode", //采购送样日期 C
        品名: "ItemName", //物料编码 D
        规格: "ItemSpecification", //物料名称 E
        光源型号: "LightSpecification", //规格型号 F
        说明: "Description", //规格型号 F
        价格: "ItemPrice", //规格型号 F
        备注: "Remark", //规格型号 F
        ID: "ID",
      };
