 ...
 * @LastEditTime: 2023-04-14 13:43:50
 * @LastEditors: max
 * @Description: 
 * @FilePath: /up-admin/src/pages/home/spectrum/price/index.vue
-->
<template>
  <div>
    <a-spin tip="导出中..." :spinning="isExportLod">
      <a-card class="card" :bordered="false" :bodyStyle="{ padding: '5px' }">
            ...
            <a-row>
              ...
              <a-col :md="6" :sm="24">
                <a-form-item label="品牌" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
                  <a-input style="width: 200px" allowClear placeholder="请输入品牌" v-decorator="['brand']" />
                </a-form-item>
              </a-col>
              <a-col :md="6" :sm="24">
                <a-form-item label="品号" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
                  <a-input style="width: 200px" allowClear placeholder="请输入品牌" v-decorator="['itemcode']" />
                </a-form-item>
              </a-col>
              <a-col :md="6" :sm="24">
                <a-form-item label="品名" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
                  <a-input style="width: 200px" allowClear placeholder="请输入品牌" v-decorator="['itemname']" />
                </a-form-item>
              </a-col>
              <a-col :md="6" :sm="24">
              ...
              <a-col :md="6" :sm="24">
                <a-form-item label="光源型号" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
                  <a-input style="width: 200px" allowClear placeholder="请输入光源型号" v-decorator="['lightspecification']" />
                </a-form-item>
              </a-col>
              <a-col :md="6" :sm="24">
                <a-form-item label="说明" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18, offset: 1 }">
                  <a-input style="width: 200px" allowClear placeholder="请输入说明" v-decorator="['description']" />
                </a-form-item>
              </a-col>
            </a-row>
          </div>
        ...
        <div class="operator">
          ...
          <a-button v-if="hasPerm('import')" icon="import" type="primary" :loading="loading" @click="isUpdateExcel = true" style="margin-left: 8px">更新数据</a-button>
      ...
      <updateExcel v-if="isUpdateExcel" @closeModal="closeModal" @success="searchBtn" />
    </a-spin>
  </div>
</template>

<script>
import { getSpectrumPrice, setSpectrumPrice } from "@/services/web.js";
...
export default {
  mixins: [PublicVar],
  components: { useForm, ImportExcel, updateExcel },
  data() {
    return {
      scrollY: "",
      advanced: true,
  ...
  methods: {
    ...
    getEnterList() {
      getSpectrumPrice({ entertypecode: "COMPANY" }, "getenterlist").then((res) => {
        if (res.data.success) {
    ...
    search() {
      this.searchForm.validateFields((err, values) => {
        if (!err) {
          ...
          getSpectrumPrice(params, "list").then((res) => {
            if (res.data.success) {
    ...
    allDel() {
      ...
      self.$confirm({
        ...
        onOk() {
          ...
          setSpectrumPrice(params, "delete").then((res) => {
            if (res.data.success) {
