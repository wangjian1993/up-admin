...
import { setSpecimenLocation } from "@/services/web.js";
...
export default {
  ...
  methods: {
    ...
    submitExcel(params) {
      this.isUpload = true;
      setSpecimenLocation(params, "import").then((res) => {
        if (res.data.success) {
    ...
    readFile(file) {
      ...
      const userRelations = {
        ...
        储存位置: "StoredLocation",
        OA是否作废: "IsOaNg", 
        是否从IQC回收签样: "IsIqcRecovery", 
        所回收的资料登记: "RecoveryInfo",
        备注: "Remark", 
      };
      let tableHead = [];
