 ...
 * @LastEditTime: 2023-08-12 14:00:23
 * @LastEditors: max
 * @Description: 
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      SupplierCode: (start) => start.specimen.SupplierCode,
    }),
  },
  created() {
    this.getUserForm();
    if (this.isEdit) {
      ...
      this.isInputCode = this.editData.ItemCode === "材质样承认" ? true : false;
      if (!this.isEditBnt) {
        this.form.Purchaser = this.Purchaser;
        this.form.Supplier = this.Supplier;
        this.form.SupplierCode = this.SupplierCode;
