 ...
 * @LastEditTime: 2023-04-25 14:56:01
 * @LastEditors: max
 * @Description: 
 * @FilePath: /up-admin/src/pages/home/production/editProcess/outForm.vue
-->
<template>
  <div>
...
<script>
import { setEditOutbound } from "@/services/web.js";
export default {
  props: ["editData"],
  data() {
    return {
    ...
    handleOk() {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          setEditOutbound(params, "update").then((res) => {
            if (res.data.success) {
              this.$emit("closeModal");
              this.$emit("succeed");
